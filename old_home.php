...
<body>
    ...
    $spotlightQuery = "SELECT * FROM properties WHERE FIND_IN_SET('Sale', available_for)  AND property_status = 'Spotlight' 
    LIMIT 1";
    ...
    <!-- ------Focus projects------- -->
    <?php
    $focusQuery = "SELECT * FROM properties  WHERE FIND_IN_SET('Sale', available_for)  AND  property_status = 'Focus' 
    LIMIT 2"; 
    ...
    <?php
    $trendingQuery = "SELECT * FROM properties  WHERE FIND_IN_SET('Sale', available_for)  AND  approval_status = 'Approved' ORDER BY id DESC";
    ...
    <?php
    $focusQuery = "SELECT * FROM properties  WHERE FIND_IN_SET('Sale', available_for)  AND  property_status = 'Sale & Commercial' LIMIT 2";
    $result = mysqli_query($conn, $focusQuery);
    ...
    <!-- <?php
            $trendingQuery = "SELECT * FROM properties WHERE property_status = 'Trending' LIMIT 6";
            $trendingResult = mysqli_query($conn, $trendingQuery);
            if ($trendingResult && mysqli_num_rows($trendingResult) > 0) {
    ...
    <?php
    // Query to fetch properties available for sale
    ...
    <section class="trending">
        <div class="container">
            ...
            <?php if ($saleResult && mysqli_num_rows($saleResult) > 0) { ?>
                <div class="grid">
                    <?php while ($row = mysqli_fetch_assoc($saleResult)) {
                        $images = explode(',', $row['file_upload']);
                    ?>
                        <a href="users/property-results.php?id=<?php echo (int)$row['id']; ?>" class="card-link">
                            <div class="card-project">
            ...
            <?php } else { ?>
                <p class="no-properties">No properties available for sale at the moment.</p>
            <?php } ?>
        </div>
    </section>
    ...
    $featuredQuery = "SELECT * FROM properties  WHERE FIND_IN_SET('Sale', available_for)  AND  property_status = 'Featured' LIMIT 6"; // Adjust LIMIT as needed
