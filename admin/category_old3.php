...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['property_type'])) {
    ...
    } elseif ($commercial_rent_from !== null && $commercial_rent_to !== null) {
        // Insert for commercial property rent
        $sql = "INSERT INTO category (property_choose, property_type, commercial_rent_from, commercial_rent_to) 
                VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssdd", $property_choose_str, $property_type, $commercial_rent_from, $commercial_rent_to);
    } elseif ($expected_deposit_from !== null && $expected_deposit_to !== null) {
        // Insert for property with deposit details
