...
<body>
    ...
    ?>
            ...
            <div class="filters-btn2">
                <a href="commercial_search_property">Less Filters</a>
            </div>
        ...
        <section class="locations-property">
            ...
            value="<?php echo htmlspecialchars($state); ?>" > -->
                    ...
                    <button type="button" class="btn" onclick="window.location.href='commercial_property2'">
                        <i class="fa fa-refresh plus_location" aria-hidden="true"></i>
                    </button>
                </form>
            </div>
        ...
        // Initialize search parameters from POST
        ...
        if (!empty($location)) {
            $where_clauses[] = "area LIKE '%$location%'";
        }
        if (!empty($city)) {
            $where_clauses[] = "city LIKE '%$city%'";
        }
        if (!empty($state)) {
            $where_clauses[] = "state LIKE '%$state%'";
        }
        if (!empty($bhkType)) {
            $where_clauses[] = "REPLACE(bhk_type, ' ', '') = REPLACE('$bhkType', ' ', '')";
        }
        ...
        if (!empty($furnishing)) {
            $where_clauses[] = "furnishing='$furnishing'";
        }


        if (!empty($propertyType)) {
            $normalizedPropertyType = str_replace(' ', '', strtoupper($propertyType));
            $where_clauses[] = "REPLACE(UPPER(property_type), ' ', '') LIKE '%$normalizedPropertyType%'";
        }

        // Filter-based conditions
        if (!empty($preferredTenants)) {
            $tenant_clauses = array_map(function ($tenant) {
                return "preferred_tenants LIKE '%" . sanitize($tenant) . "%'";
            }, $preferredTenants);
            $where_clauses[] = "(" . implode(" OR ", $tenant_clauses) . ")";
        }
        if (!empty($floors)) {
            $floor_clauses = array_map(function ($floor) {
                return "floor = '" . sanitize($floor) . "'";
            }, $floors);
            $where_clauses[] = "(" . implode(" OR ", $floor_clauses) . ")";
        }
        if (!is_null($minArea)) {
            $where_clauses[] = "build_up_area >= $minArea";
        }
        if (!is_null($maxArea)) {
            $where_clauses[] = "build_up_area <= $maxArea";
        }
        ...
        if (!empty($parkingOptions)) {
            $parking_clauses = array_map(function ($parking) {
                return "parking = '" . sanitize($parking) . "'";
            }, $parkingOptions);
            $where_clauses[] = "(" . implode(" OR ", $parking_clauses) . ")";
        }
        if (!empty($amenities)) {
            $amenity_clauses = array_map(function ($amenity) {
                return "amenities LIKE '%" . sanitize($amenity) . "%'";
            }, $amenities);
        ...
        <script>
            ...
            function resetSearch() {
                ...
                window.location.href = 'commercial_property2'; // Reload property.php to reset search
            }
